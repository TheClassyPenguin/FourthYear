...
(deffacts ejemplo
	(Personaje(nombre_Personaje Desconocido))
...
(defrule Necromancer (declare (salience 100)(no-loop TRUE))
    ?personaje<- (Personaje (nombre_Personaje ?nombre)(deduction nil)(int yes)(wit yes)(physical yes)(bleed yes)(disease yes)(medium yes)(summoner yes))
    =>	(printout t ?nombre " es un Necromancer" ?newline ?newline)
    	(modify ?personaje (deduction yes)))

(defrule Polymorph (declare (salience 100)(no-loop TRUE))
    ?personaje<- (Personaje (nombre_Personaje ?nombre)(deduction nil)(wit yes)(physical yes)(medium yes)(crowdControl yes)(buffer yes))
...
(defrule longMage ;(no-loop TRUE)
    ...
    =>  (modify ?personaje (long yes))
    	(printout t ?nombre " es mago y por lo tanto tiene largo alcance" ?newline))
...
(defrule magicNOTphysicalButElemental ;(no-loop TRUE)
    ...
    =>  (modify ?personaje (physical no)(elemental yes))
    	(printout t ?nombre " usa daño mágico y por lo tanto no usa daño físico pero si elemental" ?newline))
